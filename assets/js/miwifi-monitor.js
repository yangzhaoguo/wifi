(function () {
    ...
    var version = "v1.3.2 (2014.11.21)", guidCookieDomains = [];
    var MIWIFI_MONITOR = (function (window, undefined) {
        ...
        var doc = document, nav = navigator, screen = window.screen,
            domain = isLocal ? "" : document.domain.toLowerCase(), ua = nav.userAgent.toLowerCase();
        var StringH = {
            trim: function (s) {
            ...
            getProject: function () {
                ...
                function guid() {
                    var s = [nav.appName, nav.version, nav.language || nav.browserLanguage, nav.platform, nav.userAgent, screen.width, "x", screen.height, screen.colorDepth, doc.referrer].join(""),
                        sLen = s.length, hLen = window.history.length;
                    while (hLen) {
                        s += (hLen--) ^ (sLen++)
                    }
                ...
                var guidKey = "__guid", id = Cookie.get(guidKey);
                if (!id) {
                    id = [hash(isLocal ? "" : doc.domain), guid(), +new Date + Math.random() + Math.random()].join(".");
                    var config = {expires: 24 * 3600 * 1000 * 300, path: "/"};
                    if (guidCookieDomains.length) {
                        for (var i = 0; i < guidCookieDomains.length; i++) {
                            var guidCookieDomain = guidCookieDomains[i], gDomain = "." + guidCookieDomain;
                ...
                var countKey = "monitor_count", count = Cookie.get(countKey);
        ...
        var data = {
            getBase: function () {
                ...
                return {b: util.getBrowser(), c: util.getCount(), r: util.getReferrer(), fl: util.getFlashVer()}
            }, getClick: function (e) {
                e = EventH.fix(e || event);
                var target = e.target;
                if (target.attributes && ("data-log-element" in target.attributes)) {
                    return {element: target.attributes["data-log-element"]}
                }
                return false
                ...
                var target = e.target, tagName = target.tagName, containerId = util.getContainerId(target);
                if (tagName == "INPUT") {
                    var form = NodeH.parentNode(target, "FORM");
                    if (form) {
                        var formId = form.id || "", tId = target.id,
                            result = {f: form.action, c: "form:" + (form.name || formId), cId: containerId};
        ...
        var config = {trackUrl: null, clickUrl: null, areaIds: null};
        var $ = function (str) {
            return document.getElementById(str)
        };
        return {
            version: version, util: util, data: data, config: config, sendLog: (function () {
                window.__miwifi_monitor_imgs = {};
                return function (url) {
                    var id = "log_" + (+new Date), img = window.__miwifi_monitor_imgs[id] = new Image();
