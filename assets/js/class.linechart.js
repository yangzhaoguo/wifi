(function () {
    function getAnchors(p1x, p1y, p2x, p2y, p3x, p3y) {
        var l1 = (p2x - p1x) / 2, l2 = (p3x - p2x) / 2, a = Math.atan((p2x - p1x) / Math.abs(p2y - p1y)),
            b = Math.atan((p3x - p2x) / Math.abs(p2y - p3y));
        a = p1y < p2y ? Math.PI - a : a;
        b = p3y < p2y ? Math.PI - b : b;
        var alpha = Math.PI / 2 - ((a + b) % (Math.PI * 2)) / 2, dx1 = l1 * Math.sin(alpha + a),
            dy1 = l1 * Math.cos(alpha + a), dx2 = l2 * Math.sin(alpha + b), dy2 = l2 * Math.cos(alpha + b);
    ...
    Linechart.prototype = {
        init: function (options) {
            ...
            var r = this.svgPaper, maxYnum = this.maxYnum, maxYval = this.maxYvalText,
                perHeight = this.height / maxYnum, width = this.width, gridY = r.set();
            for (var i = 0; i < maxYnum; i++) {
                var posMY = perHeight * i, posLY = width;
                gridY.push(r.path("M0 " + posMY + "L" + posLY + " " + posMY + "Z"))
            }
            gridY.attr({stroke: "#d9d9d9", "stroke-dasharray": ". "})
        }, drawLabel: function () {
            var r = this.svgPaper, maxYnum = this.maxYnum, maxYval = parseInt(this.maxYvalText),
                perYval = parseInt(maxYval / maxYnum), perHeight = this.height / maxYnum, labelUnit = this.labelUnit,
                labelY = r.set();
            for (var i = 0; i < maxYnum; i++) {
            ...
            var r = this.svgPaper, isvml = Raphael.vml, index = index || 0, line_color = this.lineColor[index],
                dot_color = this.dotColor[index], area_color = this.areaColor[index], height = this.height,
                width = this.width, maxYnum = this.maxYnum, X = width / (data.length - 1), Y = height / this.maxYval,
                path = r.path().attr({stroke: line_color, "stroke-width": 2, "stroke-linejoin": "round"}),
                bgp = r.path().attr({stroke: "none", opacity: 0.5, fill: area_color}), blanket = r.set(), dot = r.set();
            var p, bgpp;
            for (var i = 0, ii = data.length; i < ii; i++) {
                data[i] = data[i] > this.maxYval ? this.maxYval : data[i];
                var y = Math.round(height - Y * data[i]), x = Math.round(X * i);
                if (!i) {
                    p = ["M", x, y, "C", x, y];
                    bgpp = ["M", X * i, height, "L", x, y, "C", x, y]
                }
                if (i && i < ii - 1) {
                    var Y0 = Math.round(height - Y * data[i - 1]), X0 = Math.round(X * (i - 1)),
                        Y2 = Math.round(height - Y * data[i + 1]), X2 = Math.round(X * (i + 1));
            ...
            if (max < 250 * 1000) {
                ...
                if (max > 250 * 1000 && max < 5 * 1000 * 1000) {
